...
const DynamicWidthText: React.FC<{ text: string; width: string; isOpen: boolean; onClick: () => void; isPlaceholder: boolean }> = ({ text, width, isOpen, onClick, isPlaceholder }) => {
    return (
        <div
...
const ReactSelectable: React.FC<ReactSelectableProps> = ({ options, setvalue, selected, value, fieldName, label, direction = 'bottom' }) => {
    ...
    return (
        <div ref={dropdownRef} className="relative  w-full origin-center">
            <DynamicWidthText isOpen={isOpen} text={selectedLabel} width={dropdownWidth} onClick={handleToggle} isPlaceholder={isPlaceholder} />
            {isOpen && (
                <div
                    className={`absolute w-full origin-bottom flex flex-col items-start gap-[5px] rounded-[10px] shadow-2xl bg-white z-10 ${positionClass}`}
                >
                    {options.map((option) => (
                        <div
                            key={option.value}
                            className="cursor-pointer w-full flex justify-center py-[5px] items-center whitespace-nowrap text-left hover:bg-gray-200"
